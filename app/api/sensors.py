...
def pat_sensor():
    try:
        json = request.json
        place=Place.query.filter_by(name=json['update']['place']).first()
        type=Collect_type.filter_by(name=json['update']['type']).first()
        col = Collector.query.filter_by(id=json['sensor_id']).first()
        if place: col.collector_place=place
        if type: col.collector_type=type
        db.session.add(col)
        db.session.commit()
    ...
    try:
        ...
        if place or state:
            if place and not state:
                ...
                cols = Collector.query.filter_by(collector_place=p).offset(offset).limit(limit)
            if state and not place:
                s = Collect_state.query.filter_by(name=state).first()
                cols = Collector.query.filter_by(collector_state=s).offset(offset).limit(limit)
            if place and state:
                p = Place.query.filter_by(name=place).first()
                s = Collect_state.query.filter_by(name=state).first()
                cols = Collector.query.filter_by(collector_place=p,collector_state=s).offset(offset).limit(limit)

            data=[]
            for col in cols:
                da={"id":col.id,"name":col.name,"type":col.type,"place":col.place,"state":col.state,"run_time":col.run_time}
                data.append(da)

        elif uid:
            col = Collector.query.filter_by(id=uid).first()
            data=[{"id":col.id,"name":col.name,"type":col.type,"place":col.place,"state":col.state,"run_time":col.run_time}]
        else:
